...
def translines_nlp(net, demand, T=15., lr=1., L=1., C=1., R=1e-3, G=2e-3,
    ...
    for i, consumer in enumerate(consumers):
        lines = [line for line, end_node in enumerate(end_vertices) if end_node == consumer]
        # box rule (consistent with [1] up to a scaling with dt)
        for t in range(nt-1):
            J = J + 0.5 * dt * (sum(xi_p[line][-1,t] for line in lines) - demand[i,t])**2
...
def plot_solution(net, u, alpha, xi_p, xi_m, demand, T, nt):
    ...
    plt.figure(1).clear()
    fig, axes = plt.subplots(n_ctrls, 1, num=1, figsize=(10, 2.2*n_ctrls))
    ...
    plt.figure(2).clear()
    fig, axes = plt.subplots(n_confg+1, 1, num=2, figsize=(10, 2.5*(n_confg+1)))
    axes = axes.reshape((-1,))
    ...
    plt.figure(3).clear()
    fig, axes = plt.subplots(m, n, num=3, figsize=(3.2*n, 2.8*m))
    ...
    plt.figure(4).clear()
    fig, axes = plt.subplots(m, n, num=4, figsize=(3.2*n, 2.8*m))
    axes = axes.reshape((-1,)) if m > 1 or n > 1 else [axes]
    for i in range(n_edges):
    ...
    plt.figure(5).clear()
    fig, axes = plt.subplots(n_consumer, 1, num=5, figsize=(10, 2.2*n_consumer))
